...
public class MainActivity extends Activity implements ProductListAdapterListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnCheckout.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Intent i = new Intent();
				i.setClass(MainActivity.this,Main.class);
				startActivity(i);
		...
		StringRequest verifyReq = new StringRequest(Method.POST,
				Config.URL_VERIFY_PAYMENT, new Response.Listener<String>() {
					...
					public void onErrorResponse(VolleyError error) {
						...
						Toast.makeText(getApplicationContext(),
								"payment Successfull", Toast.LENGTH_LONG).show();
						// hiding the progress dialog
						hidepDialog();
					}
				}) {
	...
	public void onAddToCartPressed(Product product) {
		...
		Toast.makeText(getApplicationContext(),
				item.getName() + " has been foward to the paypal!", Toast.LENGTH_SHORT).show();

		// Check for empty cart
		if (productsInCart.size() > 0) {
			launchPayPalPayment();
		} else {
			Toast.makeText(getApplicationContext(), "Bill is empty.",
					Toast.LENGTH_SHORT).show();
		}
